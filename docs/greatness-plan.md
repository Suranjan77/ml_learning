...
needs a repeatable one-sentence causal takeaway, successful human observation,
and no mismatch between visual implication and actual computation.
...
G2 passes when most observed participants can explain how local slope, learning
rate, and surface geometry changed the outcome in their own words, and the
intended memory image appears in recall.

## Release order

1. G0: preserve the baseline, freeze expansion, and define the gate.
2. G1: make Gradient Descent pass the scorecard.
3. G2: observe real users and correct the design.
...
- Performance budgets, unit tests, browser checks, and viewport checks remain
  release gates.
- No 14th exhibit until Gradient Descent passes G1 and G2.

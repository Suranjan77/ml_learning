...
function MatrixGrid({ matrix, x, y, size, selected, onSelect, label }: {
  ...
  return <g>
    <text x={x} y={y - 12} fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>{label}</text>
    {matrix.map((row, rowIndex) => row.map((value, columnIndex) => {
      ...
      return <g key={`${rowIndex}-${columnIndex}`} onClick={() => onSelect?.(rowIndex, columnIndex)} className={onSelect ? "cursor-pointer" : undefined}>
        ...
        {size >= 35 ? <text x={x + columnIndex * size + (size - 2) / 2} y={y + rowIndex * size + size / 2 + 4} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={Math.abs(value) / maximum > 0.62 ? vizTokens.pointOutline : vizTokens.ink}>{value}</text> : null}
      </g>;
    }))}
  </g>;
}
    ...
    <section aria-label="CNN convolution visualisation" className="grid h-full min-h-[22rem] grid-rows-[minmax(0,1fr)_auto] overflow-hidden border border-outline bg-surface">
      ...
      <div role="img" aria-labelledby={titleId} aria-describedby={instructionId} tabIndex={0} onKeyDown={moveCell} className="min-h-0 overflow-x-auto overflow-y-hidden focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary">
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full min-w-[700px] sm:min-w-0 sm:w-full" aria-hidden="true">
          ...
          <text x="563" y="292" textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>THE SAME NINE WEIGHTS SLIDE</text>
          {activeStep < 3 ? (
            <>
              ...
              <text x="563" y="476" textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.caption} fill={vizTokens.path}>Σ NINE PRODUCTS = {response}</text>
            </>
          ) : null}
              ...
              {Array.from({ length: 9 }, (_, index) => {
                ...
                return (
                  <g key={`pool-source-${index}`}>
                    ...
                    <text x={706 + column * 78} y={130 + row * 78} fontFamily="var(--font-mono)" fontSize={type.label} fill={vizTokens.path}>{index + 1}</text>
                  </g>
                );
              })}
              <path d="M941 234 H962" stroke={vizTokens.axis} strokeWidth="2" />
              <path d="M962 234 l-8 -5 v10 z" fill={vizTokens.axis} />
              <text x="951" y="219" textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.label} fill={vizTokens.mutedInk}>MAX</text>
              <MatrixGrid matrix={pooled} x={974} y={145} size={60} label="3×3 POOLED MAP" />
              <text x="817" y="372" textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.path}>NINE OUTLINED 2×2 BLOCKS</text>
              <text x="1064" y="350" textAnchor="middle" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.path}>ONE MAX FROM EACH BLOCK</text>
            </>
          )}

          <g transform="translate(38 460)">
            <text fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>SELECTED CELL [{selected.row}, {selected.column}]</text>
            {activeStep >= 2 && activeStep < 3 ? <text x="695" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={response < 0 ? vizTokens.error : vizTokens.classA}>RAW {response} → ReLU → {Math.max(0, response)}</text> : null}
            {activeStep === 3 ? <text x="447" fontFamily="var(--font-mono)" fontSize={type.labelStrong} fill={vizTokens.path}>SAME ACTIVATION MAP → LESS POSITION DETAIL</text> : null}
          </g>
        </svg>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-outline bg-surface-container-low px-3 py-2">
        <span className="mr-1 font-mono viz-label uppercase tracking-label text-on-surface-variant">Filter</span>
        ...
        <p className="ml-auto hidden max-w-lg text-xs text-on-surface-variant md:block">Click an output cell to reveal the exact 3×3 input patch that produced it.</p><span className="ml-auto font-mono viz-label uppercase text-on-surface-variant sm:hidden">Swipe diagram →</span>
      </div>
    </section>
  );
}

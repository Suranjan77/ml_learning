  ...
  return <section aria-label="Regression parameter visualisation" className="grid h-full min-h-[22rem] grid-rows-[minmax(0,1fr)_auto] overflow-hidden border border-outline bg-surface">
    <div role="img" aria-labelledby={titleId} aria-describedby={instructionId} className="relative min-h-0 overflow-hidden">
      ...
      <svg ref={svgRef} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full w-full touch-none" aria-hidden="true">
        ...
        }) : CLASSIFICATION_POINTS.map((point, index) => <g key={index}><circle cx={sx(point.x)} cy={sy(point.y)} r="7" fill={point.label === 1 ? vizTokens.classA : vizTokens.classB} stroke={vizTokens.pointOutline} strokeWidth="2" /><text x={sx(point.x)} y={sy(point.y) + 3} textAnchor="middle" fontSize={type.micro} fill={vizTokens.canvas}>{point.label}</text></g>)}
        ...
        <text x={PLOT.left + 10} y={PLOT.top + 18} fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>{mode === "linear" ? "RESIDUAL LENGTH CONTRIBUTES TO SQUARED LOSS" : "ABOVE LINE → CLASS 1 · BELOW LINE → CLASS 0"}</text>

        <text x={MAP.left} y={28} fontFamily="var(--font-dm-mono)" fontSize={type.caption} fill={vizTokens.mutedInk}>LOSS CONTOURS · DRAG THE CROSSHAIR</text>
        <text x={MAP.left + MAP.width} y={46} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.label} fill={vizTokens.mutedInk}>LIGHT LOW LOSS → DARK HIGH LOSS</text>
        ...
        <text x={MAP.left - 13} y={MAP.top + 4} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.label} fill={vizTokens.mutedInk}>+2</text>
        <text x={MAP.left - 13} y={MAP.top + MAP.height / 2 + 3} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.label} fill={vizTokens.mutedInk}>0</text>
        <text x={MAP.left - 13} y={MAP.top + MAP.height + 3} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.label} fill={vizTokens.mutedInk}>−2</text>
        <text x={MAP.left - 40} y={MAP.top + MAP.height / 2} transform={`rotate(-90 ${MAP.left - 40} ${MAP.top + MAP.height / 2})`} textAnchor="middle" fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>INTERCEPT b</text>
        <text x={MAP.left} y={MAP.top + MAP.height + 24} fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>−1.5</text>
        <text x={MAP.left + MAP.width / 2} y={MAP.top + MAP.height + 24} textAnchor="middle" fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>SLOPE m = 0</text>
        <text x={MAP.left + MAP.width} y={MAP.top + MAP.height + 24} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>+1.5</text>
        <text x={MAP.left} y={MAP.top + MAP.height + 54} fontFamily="var(--font-dm-mono)" fontSize={type.body} fill={vizTokens.ink}>LOSS {currentLoss.toFixed(3)}</text>
        {accuracy !== null ? <text x={MAP.left + MAP.width} y={MAP.top + MAP.height + 54} textAnchor="end" fontFamily="var(--font-dm-mono)" fontSize={type.body} fill={vizTokens.classA}>ACCURACY {Math.round(accuracy * 100)}%</text> : null}
        <rect
          x={MAP.left}
          y={MAP.top}
          width={MAP.width}
      ...
      <div aria-hidden="true" className="absolute bottom-2 left-2 right-2 flex items-center justify-between gap-3 border border-outline bg-surface/95 p-2 font-mono viz-label uppercase tracking-[0.06em] sm:hidden">
    ...
    <div className="grid grid-cols-[auto_1fr_1fr] items-end gap-3 border-t border-outline bg-surface-container-low p-2 sm:px-3">
      <fieldset><legend className="font-mono viz-label uppercase tracking-label text-on-surface-variant">Model</legend><div className="mt-1 flex">{(["linear", "logistic"] as const).map((item) => <button key={item} type="button" aria-pressed={mode === item} onClick={() => { setMode(item); sync({ mode: item }); }} className={`min-h-9 border px-3 text-xs capitalize ${mode === item ? "border-primary bg-primary text-on-primary" : "border-outline bg-surface"}`}>{item}</button>)}</div></fieldset>
      <label><span className="flex justify-between font-mono viz-label uppercase tracking-label text-on-surface-variant"><span>Slope</span><span>{slope.toFixed(2)}</span></span><input aria-label="Slope" type="range" {...SLOPE} value={slope} onChange={(event) => { const next = Number(event.target.value); setSlope(next); sync({ slope: next }); }} /></label>
      <label><span className="flex justify-between font-mono viz-label uppercase tracking-label text-on-surface-variant"><span>Intercept</span><span>{intercept.toFixed(2)}</span></span><input aria-label="Intercept" type="range" {...INTERCEPT} value={intercept} onChange={(event) => { const next = Number(event.target.value); setIntercept(next); sync({ intercept: next }); }} /></label>
    </div>
  </section>;
}

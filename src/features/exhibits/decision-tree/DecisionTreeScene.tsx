...
export default function DecisionTreeScene({ step, resetKey, playing = false }: ExhibitSceneProps) {
  ...
  return <section aria-label="Decision tree partition visualisation" className="grid h-full min-h-[22rem] grid-rows-[minmax(0,1fr)_auto] overflow-hidden border border-outline bg-surface">
    <div role="img" aria-labelledby={titleId} className="relative min-h-0 overflow-hidden">
      ...
      <svg ref={svgRef} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full w-full" aria-hidden="true">
        ...
        {referenceThreshold !== null && referenceThreshold !== threshold ? <>
          ...
          <text x={(sx(referenceThreshold) + sx(threshold)) / 2} y={PLOT.top + PLOT.height - 10} textAnchor="middle" fontFamily="var(--font-dm-mono)" fontSize={type.label} fill={vizTokens.path}>ROOT ROUTE CHANGED</text>
        </> : null}
        ...
        <text x={PLOT.left + 10} y={PLOT.top + 18} fontFamily="var(--font-dm-mono)" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>BACKGROUND = LEAF PREDICTION · RED OUTLINE = MISTAKE</text>
        <line
          x1={sx(threshold)}
          x2={sx(threshold)}
          y1={PLOT.top}
        ...
        <g data-testid="decision-routing-tree" transform="translate(775 42)" fontFamily="var(--font-dm-mono)">
          <text fontSize={type.caption} fill={vizTokens.mutedInk}>ROUTING RULES</text>
          ...
          <rect x="95" y="30" width="200" height="48" fill={vizTokens.canvas} stroke={vizTokens.selection} strokeWidth="2" /><text x="195" y="59" textAnchor="middle" fontSize={type.body} fill={vizTokens.ink}>x &lt; {threshold.toFixed(1)}?</text>
          {referenceThreshold !== null && referenceThreshold !== threshold ? <text x="305" y="58" fontSize={type.label} fill={vizTokens.mutedInk}>KEPT x &lt; {referenceThreshold.toFixed(1)}</text> : null}
          ...
          <text x="84" y="101" fontSize={type.label} fill={vizTokens.mutedInk}>YES</text><text x="239" y="101" fontSize={type.label} fill={vizTokens.mutedInk}>NO</text>
          ...
          <text y="390" fontSize={type.body} fill={vizTokens.ink}>{leafCount(depth)} LEAVES</text><text x="390" y="390" textAnchor="end" fontSize={type.body} fill={accuracy === 1 ? vizTokens.classA : vizTokens.error}>{Math.round(accuracy * 100)}% ACCURACY</text>
          {referenceThreshold !== null ? <><text y="414" fontSize={type.labelStrong} fill={vizTokens.path}>{reroutedPointIndices.length} REROUTED AT ROOT</text><text x="390" y="414" textAnchor="end" fontSize={type.labelStrong} fill={changedPredictionCount > 0 ? vizTokens.error : vizTokens.mutedInk}>{changedPredictionCount} PREDICTIONS CHANGED</text></> : null}
          {focusedPoint && focusedReferencePrediction && focusedCurrentPrediction ? <>
            <text y="438" fontSize={type.labelStrong} fill={vizTokens.mutedInk}>TRACE POINT {String((focusedPointIndex ?? 0) + 1).padStart(2, "0")}: KEPT {focusedReferencePrediction} → CURRENT {focusedCurrentPrediction}</text>
            <text y="455" fontSize={type.labelStrong} fill={focusedReferencePrediction === focusedCurrentPrediction ? vizTokens.classA : vizTokens.error}>{focusedReferencePrediction === focusedCurrentPrediction ? "ROOT ROUTE CHANGED; PREDICTION HELD" : "ROOT ROUTE AND PREDICTION CHANGED"}</text>
          </> : null}
        </g>
      </svg>
      <div aria-hidden="true" className="absolute bottom-2 left-2 right-2 border border-outline bg-surface/95 p-2 sm:hidden">
        <div className="flex items-center justify-between font-mono viz-label uppercase tracking-[0.06em] text-on-surface-variant">
        ...
        <p className="mt-1 viz-caption leading-4 text-on-surface">
        ...
        {focusedPoint && focusedReferencePrediction && focusedCurrentPrediction ? <p className="mt-1 font-mono viz-label text-accent">Point {String((focusedPointIndex ?? 0) + 1).padStart(2, "0")}: kept {focusedReferencePrediction} → current {focusedCurrentPrediction}</p> : null}
      </div>
    ...
    <div className="grid grid-cols-[auto_1fr] items-end gap-4 border-t border-outline bg-surface-container-low p-2 sm:px-3">
      <fieldset><legend className="font-mono viz-label uppercase tracking-label text-on-surface-variant">Tree depth</legend><div className="mt-1 flex">{([1, 2, 3] as const).map((value) => <button key={value} type="button" aria-pressed={depth === value} onClick={() => { setDepth(value); sync({ depth: value }); }} className={`min-h-9 border px-4 text-xs ${depth === value ? "border-primary bg-primary text-on-primary" : "border-outline bg-surface"}`}>{value}</button>)}</div></fieldset>
      <label><span className="flex justify-between font-mono viz-label uppercase tracking-label text-on-surface-variant"><span>Root threshold x</span><span>{threshold.toFixed(1)}</span></span><input aria-label="Root threshold" type="range" {...THRESHOLD} value={threshold} onChange={(event) => chooseThreshold(Number(event.target.value))} />
...
function Node({ x, y, label }: { x: number; y: number; label: string }) {
  const type = useVizType();
  return <g><rect x={x} y={y} width="130" height="42" fill={vizTokens.canvas} stroke={vizTokens.path} /><text x={x + 65} y={y + 26} textAnchor="middle" fontSize={type.labelStrong} fill={vizTokens.ink}>{label}</text></g>;
}

function Leaf({ x, y, label }: { x: number; y: number; label: "A" | "B" }) {
  const type = useVizType();
  return <g><rect x={x} y={y} width="80" height="42" fill={label === "A" ? vizTokens.classA : vizTokens.classB} fillOpacity="0.25" stroke={label === "A" ? vizTokens.classA : vizTokens.classB} /><text x={x + 40} y={y + 26} textAnchor="middle" fontSize={type.labelStrong} fill={vizTokens.ink}>LEAF {label}</text></g>;
}
